// (removed duplicate minimal OrdersService; keeping the full-feature implementation below)
import {
  Injectable,
  NotFoundException,
  BadRequestException,
...
interface CalculatedItem extends ItemWithMenuItem {
  unitPrice: number;
  subtotal: number;
  taxRate: number;
  taxAmount: number;
...
export class OrdersService {
  ...
  constructor(
    ...
    private saleItemRepository: Repository<OrderItem>,
    @InjectRepository(OrderPayment)
    private salePaymentRepository: Repository<OrderPayment>,
    @InjectRepository(CashRegisterSession)
    private sessionRepository: Repository<CashRegisterSession>,
    @InjectRepository(MenuItem)
    private productRepository: Repository<MenuItem>,
  ...
  async create(
    ...
    return (await this.dataSource.transaction(async (manager) => {
      ...
      const itemsWithMenuItems: ItemWithMenuItem[] = [];
      for (const itemDto of createOrderDto.items) {
        const product = await manager.findOne(MenuItem, {
          where: { id: itemDto.productId as any },
        });

        if (!product) {
          throw new NotFoundException(
            `MenuItemo ${itemDto.productId} no encontrado`,
          );
        }

        if (!product.isActive) {
          throw new BadRequestException(
            `MenuItemo ${product.name} no está activo`,
          );
        }
      ...
      const saleNumber = await this.generateOrderNumber(manager);

      // 5. Calcular totales
      const calculatedItems: CalculatedItem[] = [];
      for (const item of itemsWithMenuItems) {
      ...
      // 6. Validar pagos
      this.validatePayments(createOrderDto.payments, totals.totalAmount);
      ...
      sale.customerId = createOrderDto.customerId;
      sale.cashRegisterId = createOrderDto.cashRegisterId;
      sale.sessionId = createOrderDto.sessionId;
      ...
      sale.saleType = createOrderDto.saleType || 'RETAIL';
      sale.status = 'COMPLETED';
      sale.cashier = { id: cashierId } as any;
      sale.notes = createOrderDto.notes;

      const savedOrder = await manager.save(sale);
      ...
      for (const item of calculatedItems) {
        ...
        const saleItem = manager.create(OrderItem, {
          sale: savedOrder,
        ...
        const savedItem = await manager.save(OrderItem, saleItem);
      ...
      // 10. Crear pagos
      for (const paymentDto of createOrderDto.payments) {
  ...
  private async updateSessionTotals(
    ...
    const currentTotalOrders = Number(session.totalOrders) || 0;
    session.totalOrders = (currentTotalOrders + saleAmount).toFixed(2);
  ...
  private async generateOrderNumber(manager: EntityManager): Promise<string> {
    ...
    let sequence = 1;
    if (lastOrder) {
      const lastSequence = parseInt(lastOrder.saleNumber.slice(-6));
   ...
   * Cancelar una venta
   */
  async cancel(id: string, companyId: number, userId: string): Promise<Order> {
    return await this.dataSource.transaction(async (manager) => {
      const sale = await manager.findOne(Order, {
        where: { id },
        relations: ['items'],
      });
