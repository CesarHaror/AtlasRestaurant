...
export class CashRegisterService {
  ...
  async closeSession(
    ...
    return await this.dataSource.transaction(async (manager) => {
      ...
      const cashSalesResult = await manager
      ...
      const cashSales = Number(cashSalesResult?.total || 0);
  ...
  async getSessionReport(sessionId: string) {
    ...
    const averageTicket =
      sales.length > 0 ? Number(session.totalSales) / sales.length : 0;
    ...
    return {
      ...
      sales: {
        ...
        totalAmount: Number(session.totalSales),
